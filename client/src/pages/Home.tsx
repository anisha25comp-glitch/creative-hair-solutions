...
export default function Home() {
  ...
  const visibleMenuCategories = useMemo(() => {
    ...
    const genderSpecificItem = (itemName: string) => /men’s|beard|moustache|pre-groom|boys under 10/i.test(itemName);
    return menuCategories
      .filter((category) => menuGender === "men" ? menCategories.has(category.name) : category.name !== "Men’s grooming")
      .map((category) => ({
        ...category,
        items: category.items.filter((item) => menuGender === "men" || !genderSpecificItem(item.name)),
      }))
