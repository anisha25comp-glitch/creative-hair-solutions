...
function DashboardLayoutContent({ children, setSidebarWidth }: { children: React.ReactNode; setSidebarWidth: (width: number) => void }) {
  ...
  const visibleGroups = menuGroups.map(group => ({ ...group, items: (user?.role === "staff" ? group.items.filter(item => ["/admin/appointments", "/admin/billing", "/admin/feedback"].includes(item.path)) : group.items).map(item => ({ ...item, path: user?.role === "staff" ? item.path.replace("/admin", "/staff") : item.path })) })).filter(group => group.items.length > 0);
