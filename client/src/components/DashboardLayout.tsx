...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user) { const isStaffPortal = window.location.pathname.startsWith("/staff"); return <div className="staff-login-shell"><div className="staff-login-card"><div className="staff-login-mark">TC</div><span className="admin-kicker">The Creative · {isStaffPortal ? "Staff portal" : "Admin portal"}</span><h1>{isStaffPortal ? "Staff login" : "Admin login"}</h1><p>{isStaffPortal ? "Sign in to create appointments, generate bills and record client feedback." : "Sign in to manage branches, staff, reports, services and all salon settings."}</p><Button onClick={() => startLogin()} size="lg" className="staff-login-button">Sign in as {isStaffPortal ? "staff" : "admin"}</Button><a className="staff-login-back" href="/">Return to salon website</a></div></div>; }
